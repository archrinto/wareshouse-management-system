    ...
    <div>
        <div class="mb-6">
            <div class="mb-3 flex items-center gap-4">
                ...
                @if($role)
                    <h3 class="text-2xl font-semibold">{{ __('Edit Role') }}</h3>
                @else
            ...
            <div class="rounded-md bg-white shadow">
                <form wire:submit.prevent="submit">
                                ...
                                <div class="space-y-2 mt-2">
                                    @foreach($permissionOptions as $opt)
                                        <div class="flex items-center gap-2">
                                            <input wire:model.defer="permissions" type="checkbox" id="{{ $opt['name'] }}" value="{{ $opt['name'] }}">
                    ...
                    <div class="bg-gray-50 px-4 py-4 flex justify-between sm:px-6 border-t">
                        <a href="{{ route('role.index') }}" class="inline-flex justify-center rounded-md bg-white py-2 px-4 text-sm font-medium border border-gray-300 shadow-sm hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                            {{ __('Cancel') }}
                        </a>
