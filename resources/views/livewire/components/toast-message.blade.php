<div
    x-data="toastMessageData()"
    class="fixed right-5 bottom-5 w-80 flex flex-col gap-2"
...
>
    <template x-for="message of messages" :key="message.id">
        ...
        >
            ...
            <div x-text="message.text" class="ml-3 text-sm font-normal"></div>
            <button
                type="button"
...
<script>
    function toastMessageData() {
        return {
            messages: [
                { id: Date.now(), text: 'Success', show: true },
            ],
            remove(id) {
            ...
            add(message) {
                ...
                const timeShown = 2000 * this.messages.length
                setTimeout(() => {
                    this.remove(id)
                }, timeShown)
            }
